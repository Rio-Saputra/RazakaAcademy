...
<form action="{{ route('admin.bank-soal.store') }}" method="POST" id="form-multi-soal" enctype="multipart/form-data">
    @csrf
    ...
    <div id="soal-container">
        ...
        <div class="card soal-item" data-index="0" style="position: relative;">
            ...
            <div class="form-group">
                ...
                <textarea name="soals[0][pertanyaan]" class="form-control" rows="4" placeholder="Tuliskan pertanyaan di sini..."></textarea>
                <input type="file" name="soals[0][gambar_pertanyaan]" accept="image/*" class="form-control" style="font-size: 0.8rem; padding: 0.25rem 0.5rem; height: auto; margin-top: 0.5rem;">
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <div style="display:flex; align-items:center; margin-bottom: 0.25rem;">
                        ...
                        <input type="text" name="soals[0][opsi_a]" class="form-control" style="border-radius: 0 12px 12px 0;">
                    </div>
                    <input type="file" name="soals[0][gambar_opsi_a]" accept="image/*" class="form-control" style="font-size: 0.8rem; padding: 0.25rem 0.5rem; height: auto;">
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <div style="display:flex; align-items:center; margin-bottom: 0.25rem;">
                        ...
                        <input type="text" name="soals[0][opsi_b]" class="form-control" style="border-radius: 0 12px 12px 0;">
                    </div>
                    <input type="file" name="soals[0][gambar_opsi_b]" accept="image/*" class="form-control" style="font-size: 0.8rem; padding: 0.25rem 0.5rem; height: auto;">
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <div style="display:flex; align-items:center; margin-bottom: 0.25rem;">
                        ...
                        <input type="text" name="soals[0][opsi_c]" class="form-control" style="border-radius: 0 12px 12px 0;">
                    </div>
                    <input type="file" name="soals[0][gambar_opsi_c]" accept="image/*" class="form-control" style="font-size: 0.8rem; padding: 0.25rem 0.5rem; height: auto;">
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <div style="display:flex; align-items:center; margin-bottom: 0.25rem;">
                        ...
                        <input type="text" name="soals[0][opsi_d]" class="form-control" style="border-radius: 0 12px 12px 0;">
                    </div>
                    <input type="file" name="soals[0][gambar_opsi_d]" accept="image/*" class="form-control" style="font-size: 0.8rem; padding: 0.25rem 0.5rem; height: auto;">
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <input type="file" name="soals[0][gambar_opsi_e]" accept="image/*" class="form-control" style="font-size: 0.8rem; padding: 0.25rem 0.5rem; height: auto;">
                </div>
            </div>
            ...
            <div class="form-group tkp-points-section" style="display: none;">
                ...
                <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.25rem; background: #F1F5F9; padding: 0.5rem 0.75rem; border-radius: 8px; border: 1px solid var(--border);">
                        ...
                        <input type="number" name="soals[0][option_points_{{ strtolower($opt) }}]" min="1" max="5" value="{{ $opt === 'A' ? 5 : ($opt === 'B' ? 4 : ($opt === 'C' ? 3 : ($opt === 'D' ? 2 : 1))) }}" style="width: 50px; border: 1px solid #CBD5E1; border-radius: 4px; padding: 2px 4px; font-weight: bold; text-align: center; color:#1E293B; background:white;">
                    </div>
                    @endforeach
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Event delegation for jenis-soal select toggles
        document.addEventListener('change', function(e) {
            if (e.target && e.target.classList.contains('jenis-soal-select')) {
                const item = e.target.closest('.soal-item');
                const correctSection = item.querySelector('.correct-answer-section');
                const tkpSection = item.querySelector('.tkp-points-section');
                if (e.target.value === 'TKP') {
        ...
        btnAdd.addEventListener('click', function() {
            ...
            template.querySelectorAll('input, textarea, select').forEach(input => {
                if(input.tagName === 'SELECT') {
                    ...
                    // Reset to default TKP scale
                    const name = input.getAttribute('name');
                    if (name.includes('option_points_a')) input.value = 5;
                    else if (name.includes('option_points_b')) input.value = 4;
                    else if (name.includes('option_points_c')) input.value = 3;
                    else if (name.includes('option_points_d')) input.value = 2;
                    else if (name.includes('option_points_e')) input.value = 1;
                } else {
                    input.value = "";
                }
            ...
            // Make sure the TKP and standard sections are in initial state (hidden/visible)
            template.querySelector('.correct-answer-section').style.display = 'block';
            template.querySelector('.tkp-points-section').style.display = 'none';
