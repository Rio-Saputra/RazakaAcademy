...
foreach ($blocks as $block) {
    ...
    if (preg_match('/^\[\s*(\d+)\s*\]/u', $block, $m)) {
        ...
        if (!empty($mergedBlocks)) {
            $mergedBlocks[count($mergedBlocks) - 1] .= "\n" . $block;
            $mergedBlocks[count($mergedBlocks) - 1] = preg_replace('/^\[\s*\d+\s*\][.)\:\-\s]*/u', '', $mergedBlocks[count($mergedBlocks) - 1]);
        }
    }
}
$blocks = $mergedBlocks;

echo "Total merged blocks: " . count($blocks) . "\n";
// Let's print out the titles and indices of the first 30 parsed items
for($i = 0; $i < min(30, count($blocks)); $i++) {
    $blockTrimmed = trim($blocks[$i]);
    if (empty($blockTrimmed) || !preg_match('/^\[\s*\d+\s*\]/u', $blockTrimmed)) {
        echo "[$i] SKIPPED (doesn't start with [N])\n";
        continue;
    }
    ...
    echo "[$i] " . substr(strip_tags($questionText), 0, 70) . "...\n";
}
